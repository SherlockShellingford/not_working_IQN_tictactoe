...
class FQFAgent(rainbow_agent.RainbowAgent):
  ...
  def _train_step(self):
    ...
    if self._replay.memory.add_count > self.min_replay_history:
      ...
      print("training steps:", self.training_steps)
      print("update mod:", self.training_steps % self.update_period)
      if self.training_steps % self.update_period == 0:
        if 'sqloss' in self._runtype:
            _, _, loss, loss1, quan_value, quan, vdiff, targetperhaps = self._sess.run(self._train_op)
            print("QUANTILES>>>>>>>>>>>>",quam)
            print("QUANTILES VALUES>>>>>",quam_value)
            print("TARGET VALUES>>>>>>>>",targetperhaps)

            
        else:
            _, _, _, loss, loss1, quan_value, quan, vdiff, targetperhaps = self._sess.run(self._train_op)
            print("QUANTILES>>>>>>>>>>>>",quam)
            print("QUANTILES VALUES>>>>>",quam_value)
            print("TARGET VALUES>>>>>>>>",targetperhaps)
  ...
  def _build_train_op(self):
    ...
    print ("TARGET QUANTILE:>>>>>>", target_quantile_values)
    print ("SOURCE QUANTILE:>>>>>>", chosen_action_quantile_values)
